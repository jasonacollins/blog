...
I first wrote [a version of this post in April 2023](/posts/using-large-language-models-as-academic.qmd). A lot has changed since then in both the tools and how I use them.
...
For more mechanical lecture notes, there are some clear gains. The notes for my undergraduate behavioural economics subject (which you can access [here](https://be.jcx.au)) were produced with CoPilot assistance.
...
I also use CoPilot with [ggplot](https://ggplot2.tidyverse.org) to produce graphs for the notes. When I wanted to generate a chart showing a [probability weighting function](https://be.jcx.au/prospect-theory/probability-weighting) from prospect theory, I created an R code block and typed a comment:
